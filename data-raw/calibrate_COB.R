...
# library(DMwR,quietly=TRUE) cob: discontinued by CRAN, adding VIM::kNN() instead to get code to run
...
#   if(indexx == 4){allloss <- TRUE ; nameadd <- "4"; useyears <- 2009:2020;}
  ...
  # test
  cbind(names(df.main.orig),names(df.main)) #cob: no df.main.orig included and greyed out-- relic code not used?
  ...
  df.covars <-df.main[,c(12,15:21)] #cob: includes winter and spring columns-- doesn't really match covariates add steelhead? added column 12
  print(names(  df.covars <-df.main[,c(12,15:21)] ))
  ...
  df.covars.filled <- data.frame(df.main[,c(3,8,9,10)],kNN(df.covars)) # cob: this uses kNN to fill df.covars (including obs and covars but not all listed) also drops precip then. Added column 8 and 10 to keep for later aggregation just to attempt running code
  ...
  precip <- aggregate(precip ~ year.week, data = df.covars.filled, mean)[,2] #not in dataset based on previous calls for df.covars.filled
  ...
  qrf.steelhead  <-quantregForest(x=df.week[,predictors.steelhead],y=df.week$steelhead,
                                 ntree = 1000, #cob: changed from 1000 to 500 in BOR code
                                 importance = T,
                                 proximity = T,
                                 keep.forest = T,
                                 keep.inbag = T,
