...
write.table(csv_data, paste0(csv_path, "genetic_lad_total_loss.csv"), row.names = FALSE, sep = "|")
...
p <- jpe_genetic_lad_data %>%
  ...
  labs(
    ...
    caption = paste0("Data sources: Genetic loss provided by USBR before Water Year 2010. LAD loss from CDFW Salvage Database.\n",timestamp),
    x = "Water Year",
    y = "Total loss",
    fill = NULL
  ) +
